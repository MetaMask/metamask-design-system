...
> **Note**
>
> `workspaceName` in these commands is the `name` field within a package's `package.json`, e.g., `@metamask/design-system-react`, not the directory where it is located, e.g., `packages/design-system-react`.
...
> **Note**
>
> - `workspaceName` in the Yarn documentation is the `name` field within a package's `package.json`, e.g., `@metamask/design-system-react`, not the directory where it is located, e.g., `packages/design-system-react`.
> - `commandName` in the Yarn documentation is any sub-command that the `yarn` executable would usually take. Pay special attention to the difference between `run` vs `exec`. If you want to run a package script, you would use `run`, e.g., `yarn workspace @metamask/design-system-react run changelog:validate`; but if you want to run _any_ shell command, you'd use `exec`, e.g. `yarn workspace @metamask/design-system-react exec cat package.json | jq '.version'`.
...
2. Next, you need to connect the package to your project by overriding the resolution logic in your package manager to replace the published version of the package with the local version.
   ...
   3. Add a line to this section that mirrors the dependency entry on the left-hand side and points to the local path on the right-hand side:
      ...
      > **Example:**
      >
      > - If your project uses Yarn, `@metamask/design-system-react` is listed in dependencies at `^1.1.4`, and your clone of the `metamask-design-system` repo is at the same level as your project, add the following to `resolutions`:
      >
      >   ```
      >   "@metamask/design-system-react@^1.1.4": "file:../metamask-design-system/packages/design-system-react"
      >   ```
...
3. Add a line to this section that mirrors the dependency entry on the left-hand side and points to the preview version on the right-hand side:
   ...
   > **Example:**
   >
   > - If your project uses Yarn, `@metamask/design-system-react` is listed in dependencies at `^1.1.4`, and you want to use the preview version `1.2.3-preview-e2df9b4`, add the following to `resolutions`:
   >
   >   ```
   >   "@metamask/design-system-react@^1.1.4": "npm:@metamask-previews/design-system-react@1.2.3-preview-e2df9b4"
   >   ```
