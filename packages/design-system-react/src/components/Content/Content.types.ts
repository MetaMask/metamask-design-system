 ...
 *
 * Props-only layout for the inner list row (`title`, `avatar`, inline row
 * accessories, etc.). Extends shared props with React `Box` and text slot props.
 */
export type ContentProps = Omit<BoxProps, 'children' | 'title'> &
  ContentPropsShared & {
    /**
