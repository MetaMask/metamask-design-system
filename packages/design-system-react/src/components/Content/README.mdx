...
The root is a horizontal row with no padding or min-height floors. For padded list rows with variant-driven min-heights, use ListItem once it is available on React.

```tsx
import { Content } from '@metamask/design-system-react';

<Content title="Label" description="Secondary text" value="Value" />;
```
