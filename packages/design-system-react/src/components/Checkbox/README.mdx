...
```tsx
<Checkbox id="accept-terms" label="I accept the terms" isSelected={false} onChange={() => {}} />
```
...
```tsx
<Checkbox id="styled-label" label="Label" labelProps={{ className: 'text-muted' }} />
```
...
```tsx
<Checkbox id="styled-container" checkboxContainerProps={{className: "bg-success-default"}} />
```
