...
Use `variant` to control the visual style. `ButtonIconVariant.Default` is transparent with default icon color; `ButtonIconVariant.Filled` is muted background; `ButtonIconVariant.Floating` is the contained floating style.
