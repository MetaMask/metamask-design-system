import { Controls, Canvas } from '@storybook/addon-docs/blocks';

import * as ButtonIconStories from './ButtonIcon.stories';

# ButtonIcon

A Button Icon is a compact, icon-only button that triggers an action, designed for quick, space-efficient interactions.

```tsx
import { ButtonIcon, IconName } from '@metamask/design-system-react';

<ButtonIcon iconName={IconName.Close} ariaLabel="Close" onClick={() => {}} />;
```

<Canvas of={ButtonIconStories.Default} />

## Props

### `iconName`

Use the `iconName` prop to specify which icon to display from the design system's icon set.

<table>
  <thead>
    <tr>
      <th align="left">TYPE</th>
      <th align="left">REQUIRED</th>
      <th align="left">DEFAULT</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <code>IconName</code>
      </td>
      <td align="left">Yes</td>
      <td align="left">
        <code>undefined</code>
      </td>
    </tr>
  </tbody>
</table>

### `ariaLabel`

Required prop for accessibility. This label should clearly describe the button's action for screen readers.

<table>
  <thead>
    <tr>
      <th align="left">TYPE</th>
      <th align="left">REQUIRED</th>
      <th align="left">DEFAULT</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <code>string</code>
      </td>
      <td align="left">Yes</td>
      <td align="left">
        <code>undefined</code>
      </td>
    </tr>
  </tbody>
</table>

<Canvas of={ButtonIconStories.AriaLabel} />

### `size`

ButtonIcon supports four sizes.

Available sizes:

- `ButtonIconSize.Xs` (20px)
- `ButtonIconSize.Sm` (24px)
- `ButtonIconSize.Md` (32px)
- `ButtonIconSize.Lg` (40px)

<table>
  <thead>
    <tr>
      <th align="left">TYPE</th>
      <th align="left">REQUIRED</th>
      <th align="left">DEFAULT</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <code>ButtonIconSize</code>
      </td>
      <td align="left">No</td>
      <td align="left">
        <code>ButtonIconSize.Md</code>
      </td>
    </tr>
  </tbody>
</table>

<Canvas of={ButtonIconStories.Size} />

### `variant`

Use `variant` to control the visual style. All variants are fully rounded. `ButtonIconVariant.Default` is transparent with default icon color; `ButtonIconVariant.Filled` is a muted background; `ButtonIconVariant.Floating` is the contained floating style.

<table>
  <thead>
    <tr>
      <th align="left">TYPE</th>
      <th align="left">REQUIRED</th>
      <th align="left">DEFAULT</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <code>ButtonIconVariant</code>
      </td>
      <td align="left">No</td>
      <td align="left">
        <code>ButtonIconVariant.Default</code>
      </td>
    </tr>
  </tbody>
</table>

<Canvas of={ButtonIconStories.Floating} />
<Canvas of={ButtonIconStories.Filled} />

### `isDisabled`

ButtonIcon can be disabled.

<table>
  <thead>
    <tr>
      <th align="left">TYPE</th>
      <th align="left">REQUIRED</th>
      <th align="left">DEFAULT</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <code>boolean</code>
      </td>
      <td align="left">No</td>
      <td align="left">
        <code>false</code>
      </td>
    </tr>
  </tbody>
</table>

<Canvas of={ButtonIconStories.IsDisabled} />

### `className`

Use the `className` prop to add custom CSS classes to the component. These classes will be merged with the component's default classes using `twMerge`, allowing you to:

- Add new styles that don't exist in the default component
- Override the component's default styles when needed

<table>
  <thead>
    <tr>
      <th align="left">TYPE</th>
      <th align="left">REQUIRED</th>
      <th align="left">DEFAULT</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <code>string</code>
      </td>
      <td align="left">No</td>
      <td align="left">
        <code>undefined</code>
      </td>
    </tr>
  </tbody>
</table>

### `style`

The `style` prop should primarily be used for dynamic inline styles that cannot be achieved with `className` alone. For static styles, prefer using `className` with Tailwind classes.

<table>
  <thead>
    <tr>
      <th align="left">TYPE</th>
      <th align="left">REQUIRED</th>
      <th align="left">DEFAULT</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <code>object</code>
      </td>
      <td align="left">No</td>
      <td align="left">
        <code>undefined</code>
      </td>
    </tr>
  </tbody>
</table>

## Component API

<Controls of={ButtonIconStories.Default} />

## Migration Guide

Migrating from `ui/components/component-library/button-icon` in MetaMask Extension? See the [ButtonIcon Migration Guide](../../../MIGRATION.md#buttonicon-component) for prop mappings, before/after examples, and breaking changes.

## References

[MetaMask Design System Guides](https://www.notion.so/MetaMask-Design-System-Guides-Design-f86ecc914d6b4eb6873a122b83c12940)
