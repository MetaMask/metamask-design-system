...
<table>
  <thead>
    <tr>
      ...
      <th align="left">USE FOR</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td align="left">
        <strong>Inner title row</strong>
      </td>
      <td align="left">
        <code>titleAccessory</code>
      </td>
      <td align="left">
        Inline metadata beside the title (info icon, badge, short label)
      </td>
    </tr>
    <tr>
      <td align="left">
        <strong>Outer header row</strong>
      </td>
      <td align="left">
        <code>startIconName</code> / <code>startAccessory</code>,{' '}
        <code>endIconName</code> / <code>endAccessory</code>
      </td>
      <td align="left">
        Leading avatars or icons; trailing chevrons, actions, or custom end
        nodes
      </td>
    </tr>
  </tbody>
...
When `true`, the root becomes focusable and clickable so the full row is tappable. Hover and pressed feedback use `hover:opacity-80` and `active:opacity-80` (80% opacity). An `IconName.ArrowRight` end icon is rendered by default unless `endIconName` or `endAccessory` is provided. `role` defaults to `button` and `tabIndex` defaults to `0` when not provided. Use `onClick` for activation. When `disabled` is set, clicks and keyboard activation are ignored and `aria-disabled` is set.
