...
When `true`, the root becomes focusable and clickable so the full row is tappable. Hover and pressed feedback use the `hover:bg-hover` and `active:bg-pressed` background colors. An `IconName.ArrowRight` end icon is rendered by default unless `endIconName` or `endAccessory` is provided. `role` defaults to `button` and `tabIndex` defaults to `0` when not provided. Use `onClick` for activation. When `disabled` is set, clicks and keyboard activation are ignored and `aria-disabled` is set.
