...
| `arrowProps?: BoxProps<'div'>`                                                                             | `arrowProps?: ComponentProps<'div'>`     | narrowed    | extension Box style-utility props are no longer accepted; pass `className` for one-off styling                                                                                          |
...
- `arrowProps` accepts standard `<div>` props instead of extension Box props. Pass `className` (or any HTML attribute / `data-*` / `aria-*`) directly.
