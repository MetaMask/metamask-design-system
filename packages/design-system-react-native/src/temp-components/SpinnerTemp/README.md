...
The `SpinnerTemp` component is a loading indicator designed for React Native applications. It displays a spinning icon, optionally accompanied by text, to indicate a loading state.
...
import SpinnerTemp from '@metamask/design-system-react-native';
...
```tsx
<SpinnerTemp color={IconColor.PrimaryDefault} />
```

### Adding Loading Text

```tsx
<SpinnerTemp loadingText="Loading data..." />
```
...
- The `SpinnerTemp` component uses `react-native-reanimated` for smooth animations.
...
When contributing to the `SpinnerTemp` component:
