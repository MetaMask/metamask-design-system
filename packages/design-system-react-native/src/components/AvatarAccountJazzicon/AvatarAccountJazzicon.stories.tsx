...
import AvatarAccountJazzicon from './AvatarAccountJazzicon';
...
const meta: Meta<AvatarAccountJazziconProps> = {
  title: 'Components/AvatarAccountJazzicon',
  component: AvatarAccountJazzicon,
  argTypes: {
    size: {
      control: 'select',
      options: AvatarSize,
...
export default meta;
type Story = StoryObj<AvatarAccountJazziconProps>;
...
export const Default: Story = {
  ...
  render: (args) => {
    return (
      <AvatarAccountJazzicon {...args} address={sampleAccountAddresses[0]} />
    );
  },
};
  ...
  render: () => (
    <View style={{ gap: 16 }}>
      {Object.keys(AvatarSize).map((sizeKey) => (
        <AvatarAccountJazzicon
          address={sampleAccountAddresses[1]}
          key={sizeKey}
...
export const AddressVariations: Story = {
  render: () => (
    <View style={{ gap: 16 }}>
      {sampleAccountAddresses.map((addressKey) => (
        <AvatarAccountJazzicon address={addressKey} key={addressKey} />
      ))}
    </View>
  ),
};
