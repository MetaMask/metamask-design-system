...
describe('Toast', () => {
  ...
  it('uses elevated-2 background, md shadow, and 16px radius in light theme', () => {
    const tw = renderHook(() => useTailwind()).result.current;
    ...
    expect(screen.getByTestId('toast-root')).toHaveStyle(
      tw.style('bg-elevated-2 rounded-2xl'),
    );
    expect(screen.getByTestId('toast-root')).toHaveStyle(
  ...
  it('uses elevated-2 background and no shadow in dark theme', () => {
