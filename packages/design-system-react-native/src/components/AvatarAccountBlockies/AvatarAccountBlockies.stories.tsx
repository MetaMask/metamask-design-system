...
import AvatarAccountBlockies from './AvatarAccountBlockies';
...
const meta: Meta<AvatarAccountBlockiesProps> = {
  title: 'Components/AvatarAccountBlockies',
  component: AvatarAccountBlockies,
  argTypes: {
    size: {
      control: 'select',
      options: AvatarSize,
...
export default meta;
type Story = StoryObj<AvatarAccountBlockiesProps>;
...
export const Default: Story = {
  ...
  render: (args) => {
    return (
      <AvatarAccountBlockies {...args} address={sampleAccountAddresses[0]} />
    );
  },
};
  ...
  render: () => (
    <View style={{ gap: 16 }}>
      {sampleAccountAddresses.map((addressKey) => (
        <AvatarAccountBlockies address={addressKey} key={addressKey} />
      ))}
    </View>
  ),
};
