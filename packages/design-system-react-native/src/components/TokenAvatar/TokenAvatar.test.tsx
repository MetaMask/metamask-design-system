...
import EthereumSVG from '../../assets/network-icons/ethereum.svg';

import { AvatarToken } from './index';

const remoteImageSrc = { uri: 'https://example.com/photo.png' };
const networkBadge = { src: EthereumSVG, name: 'Ethereum' };
...
describe('AvatarToken Component', () => {
  ...
  it('updates fallback text on image error when fallbackText is provided', () => {
    ...
    expect(avatarBase.props.children.props.children).toStrictEqual(
  ...
  it('computes backupFallbackText from name when fallbackText is not provided', () => {
    ...
    expect(avatarBase.props.children.props.children).toBe(
      getAvatarInitials('Example'),
    );
  });

  it('renders with networkBadge', () => {
    const { getByTestId } = render(
      <AvatarToken
        src={remoteImageSrc}
